...
export async function POST(request: Request) {
  ...
  if (body.itemRefs !== undefined) {
    if (
      ...
      return NextResponse.json(
        { error: "itemRefs must be an array of strings" },
        { status: 400 },
      );
    }
    if (body.itemRefs.length > 10) {
      return NextResponse.json(
        { error: "itemRefs cannot exceed 10 items" },
        { status: 400 },
      );
    }
  ...
  if (body.userRefs !== undefined) {
    if (
      ...
      return NextResponse.json(
        { error: "userRefs must be an array of strings" },
        { status: 400 },
      );
    }
    if (body.userRefs.length > 10) {
      return NextResponse.json(
        { error: "userRefs cannot exceed 10 mentions" },
        { status: 400 },
      );
    }
  ...
  if (error || !msg) {
    if (error?.message?.includes("Rate limit")) {
      return NextResponse.json(
        { error: "Slow down — only 1 message per second" },
        { status: 429 },
      );
    }
