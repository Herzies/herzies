...
describe("POST /api/trade/offer", () => {
	...
	it("preserves locked state when offer is unchanged (no-op resend)", async () => {
		...
		admin.from = vi.fn((table: string) => {
			...
			if (table === "herzies") {
				const chain = originalFrom("__empty__");
				(chain as Record<string, unknown>).then = (resolve: (v: unknown) => void) =>
					resolve({ data: { inventory_v2: { cd: 5 }, currency: 100 }, error: null });
				return chain;
			}
			return originalFrom(table);
		}) as typeof admin.from;
		...
		const updateArg = admin._updateFn.mock.calls[0]?.[0] as Record<string, unknown> | undefined;
	...
	it("resets locks when offer actually changes", async () => {
		...
		const res = await POST(fakeRequest({ tradeId: "t1", offer: { items: { cd: 2 }, currency: 0 } }));
		expect(res.status).toBe(200);

		const updateArg = admin._updateFn.mock.calls[0]?.[0] as Record<string, unknown>;
