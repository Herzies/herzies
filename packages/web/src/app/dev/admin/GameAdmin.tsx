...
function EventForm({
	...
	return (
		<form onSubmit={onSubmit} className="space-y-4">
			<div className="grid sm:grid-cols-2 gap-4">
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id ?? "new"}-ev-type`}>
						type
					</label>
					<select
				...
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id ?? "new"}-ev-title`}>
						title
					</label>
					<input
			...
			<div>
				<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id ?? "new"}-ev-desc`}>
					description
				</label>
				<input
			...
			<div className="grid sm:grid-cols-2 gap-4">
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id ?? "new"}-ev-start`}>
						starts at
					</label>
					<input
						...
						onChange={(e) =>
							setForm((f) => ({ ...f, startsAt: fromLocalDatetimeValue(e.target.value) }))
						}
						className={INPUT}
					/>
				</div>
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id ?? "new"}-ev-end`}>
						ends at
					</label>
					<input
						...
						onChange={(e) =>
							setForm((f) => ({ ...f, endsAt: fromLocalDatetimeValue(e.target.value) }))
						}
						className={INPUT}
					/>
				</div>
...
function ItemForm({
	...
	return (
		<form onSubmit={onSubmit} className="space-y-4">
			<div className="grid sm:grid-cols-2 gap-4">
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id || "new"}-item-id`}>
						id
					</label>
					<input
				...
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id || "new"}-item-name`}>
						name
					</label>
					<input
			...
			<div>
				<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id || "new"}-item-desc`}>
					description
				</label>
				<input
			...
			<div className="grid sm:grid-cols-3 gap-4">
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id || "new"}-item-rarity`}>
						rarity
					</label>
					<select
				...
				<div>
					<label className="block text-xs text-text-dim mb-1" htmlFor={`${form.id || "new"}-item-sell`}>
						sell price
					</label>
					<input
...
function EventRow({
	...
	const remove = async () => {
		...
		setError(null);
		try {
			await adminFetch(`/api/admin/events?id=${event.id}`, secret, { method: "DELETE" });
			onChange();
		} catch (e) {
			setError(e instanceof Error ? e.message : "Failed");
		} finally {
		...
		<>
			<tr className="border-t border-border align-top">
				<td className="py-3 pr-4">
					...
					{event.description && (
						<div className="text-text-dim text-xs mt-1 max-w-xs">{event.description}</div>
					)}
				</td>
				<td className="py-3 pr-4">
				...
				<td className="py-3 text-right whitespace-nowrap">
					...
					{error && !editing && <div className="text-red text-xs mt-1">{error}</div>}
				</td>
			</tr>
			{editing && (
...
function ItemRow({
	...
	const remove = async () => {
		...
		setError(null);
		try {
			await adminFetch(`/api/admin/items?id=${item.id}`, secret, { method: "DELETE" });
			onChange();
		} catch (e) {
			setError(e instanceof Error ? e.message : "Failed");
		} finally {
...
export function GameAdmin() {
	...
	const [eventForm, setEventForm] = useState<EventFormState>(() => newEventForm());
	...
	if (!secret) {
		return (
			<section className="border border-border rounded-sm p-6 bg-bg-panel max-w-md">
				...
				<label className="block text-xs text-text-dim mb-2" htmlFor="admin-secret">
					Admin secret
				</label>
				<input
					id="admin-secret"
	...
	return (
		<div className="space-y-10">
					...
					<table className="w-full text-sm text-left">
						...
						<tbody>
							{items.length === 0 ? (
								...
								items.map((item) => (
									<ItemRow key={item.id} item={item} secret={secret} onChange={load} />
								))
							)}
						</tbody>
					</table>
			...
			<section>
				...
				<h3 className="text-xs text-text-dim mb-2 uppercase tracking-wide">running & upcoming</h3>
				<EventsTable
					events={running}
					secret={secret}
					onChange={load}
