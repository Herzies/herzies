/**
 * Vendored copy of `packages/web/src/lib/game-server.ts` for the Edge Function.
 *
 * WHY A COPY: `supabase functions serve` and `supabase functions deploy` only
 * have access to files under `supabase/`. The runtime container can't reach
 * `../../packages/...`, so the function must be self-contained. Raw `deno run`
 * worked only because it had full filesystem access.
 *
 * Keep this in sync with the Next.js copy until the shared move below happens.
 *
 * PRODUCTION PATH: extract this game logic into a published package (e.g. move
 * it into `@herzies/shared`, or a new `@herzies/game` package on npm/JSR) and
 * import it by name from both the Next.js route and this function via a `npm:`
 * / `jsr:` specifier. That removes the duplication entirely.
 */
import {
...
async function fetchServerMultipliers(
...
export async function processSync(
  ...
  // set on the deployed function (it would spam every real user with a drop
  // on every sync) — only set it via `supabase functions serve --env-file`
  // for local testing. Deliberately skips drop_rolls_done bookkeeping in
  // test mode so toggling it off resumes normal cadence unaffected.
  const dropTestMode = Deno.env.get("HERZIES_DROP_TEST_MODE") === "1";
  ...
  if (dropTestMode || totalDropRollsEligible > dropRollsDone) {
    ...
    if (!dropTestMode) {
      await admin
        .from("herzies")
        .update({ drop_rolls_done: totalDropRollsEligible })
        .eq("user_id", userId);
    }
  }
  ...
  let pendingTradeRequest:
    | { tradeId: string; fromName: string; fromFriendCode: string }
    | undefined;
